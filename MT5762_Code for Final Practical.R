...
sd_pooled <- sqrt(((var(data$weekend==0) * (length(data$weekend==0) - 1)) + (var(data$weekend==1) * (length(data$weekend==1) - 1))) / (length(data) - 2))
...
#use the predict() function to estimate the number of ice cream sales under specific conditions. 
